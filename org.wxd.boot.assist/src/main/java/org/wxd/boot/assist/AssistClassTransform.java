...
public class AssistClassTransform implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classFileBuffer) {
        try {
            ...
            for (CtBehavior m : behaviors) {
                AssistAnn assistAnn = (AssistAnn) m.getAnnotation(AssistAnn.class);
                if (assistAnn != null && assistAnn.f()) continue;
    ...
    /** 方法增强，添加方法耗时统计 */
    private void enhanceMethod(String className, CtBehavior method) throws CannotCompileException {
        if (method.isEmpty()) {
            return;
        }
