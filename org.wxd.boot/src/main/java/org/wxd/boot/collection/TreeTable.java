...
public class TreeTable<K1 extends Comparable<K1>, K2 extends Comparable<K2>, V> implements Serializable, Data2Json {

    private final TreeMap<K1, TreeMap<K2, V>> objects;

    public TreeTable() {
        this.objects = new TreeMap<>();
    }

    public TreeTable(Map<K1, Map<K2, V>> m) {
        this();
    ...
    public TreeMap<K2, V> get(K1 key) {
        return objects.get(key);
    }

    /** 行 */
    public TreeMap<K2, V> row(K1 k1) {
        return this.objects.computeIfAbsent(k1, k -> new TreeMap<>());
    }

    public TreeMap<K2, V> put(K1 key, TreeMap<K2, V> value) {
    ...
    public TreeTable<K1, K2, V> fluentPut(K1 k1, K2 k2, V v) {
        row(k1).put(k2, v);
        return this;
    }

    public TreeMap<K2, V> remove(K1 key) {
    ...
    public void putAll(Map<? extends K1, ? extends TreeMap<K2, V>> m) {
        for (Map.Entry<? extends K1, ? extends TreeMap<K2, V>> entry : m.entrySet()) {
    ...
    public Collection<TreeMap<K2, V>> values() {
        return objects.values();
    }

    public Set<Map.Entry<K1, TreeMap<K2, V>>> entrySet() {
